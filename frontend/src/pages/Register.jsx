  ...
  return (
    <div style={s.wrapper}>  {/* ← background removed */}
      <div style={{ ...s.card, background: dark ? '#1e293b' : '#fff' }}>
        ...
        <h2 style={{ ...s.title, color: dark ? '#f1f5f9' : '#1e293b' }}>Create Account</h2>
        <p style={s.subtitle}>Join our blogging community today</p>
        <label style={{ ...s.label, color: dark ? '#94a3b8' : '#64748b' }}>Username</label>
        <input placeholder="johndoe" onChange={e => setForm({...form, username: e.target.value})}
        ...
        <input type="password" placeholder="••••••••" onChange={e => setForm({...form, password: e.target.value})}
          style={{ ...s.input, background: dark ? '#0f172a' : '#f8fafc', color: dark ? '#f1f5f9' : '#1e293b', border: `2px solid ${dark ? '#334155' : '#e2e8f0'}` }}/>
        <button onClick={handleSubmit} style={s.btn}>Create Account →</button>
        <p style={s.link}>Already have an account?{' '}
          <span onClick={() => navigate('/login')} style={s.linkText}>Sign in</span>
        </p>
      </div>
    </div>
  );
